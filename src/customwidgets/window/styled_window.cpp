...
void StyledWindow::constructHintButtons()
{
    ...
    if (layout)
    {
        ...
        rightLayoutWidget_->setMinimumWidth(close_->geometry().right()
                                            - minimize_->geometry().left());
    }
}
...
#    endif
    ...
    case WM_DPICHANGED:
    {
        ...
        {
            displayScale_ = scale;
            if (rightLayoutWidget_ && close_ && minimize_)
            {
                rightLayoutWidget_->setMinimumWidth(
                    close_->frameGeometry().right()
                    - minimize_->frameGeometry().left());
            }
            adjustSize();
            update();
        }
